...
const createSlider = () => {
  ...
  if(durationInput < 0){ 
    alert('Negative time is not expect! Please give valid time input.');
    return;
  }
...
const toggleSpinner = () => {
  ...
  const imageSelect = document.getElementById('image-container');
  imageSelect.classList.toggle('d-none');
}